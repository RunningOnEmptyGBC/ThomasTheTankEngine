...
bool Graphics::Init(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_modelBase->Init(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), L"../Resources/Models/cube.txt", L"../Resources/Textures/bricks3.tga");
	if (!result)
	{
		MessageBox(hwnd,
	...
	m_lights->SetDirection(0.0f, 0.0f, 1.0f);
	...
	result = m_bitmap->Init(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), screenWidth, screenHeight, L"../Resources/Textures/bricks3.tga", 512, 512);
	if (!result)
	{
		MessageBox(hwnd,
...
bool Graphics::Render(float rotation)
{
	...
	result = m_lightShader->Render(m_D3D->GetDeviceContext(), m_modelBase->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_modelBase->GetTexture(), m_lights->GetDirection(), m_lights->GetAmbientColour(), m_lights->GetDiffuseColour(), m_camera->GetPosition(), m_lights->GetSpecularColour(), m_lights->GetSpecularPower());
	if (!result)
	{
		return false;
	}
