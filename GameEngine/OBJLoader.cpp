	...
	bool flipFaces)
{
						...
						for (int i = 0; i < 3; ++i)      // First three vertices (first triangle)
						{
							...
							for (int j = 0; j < VertDef.length(); ++j)
							{
								...
								if (VertDef[j] == '/' || j == VertDef.length() - 1)
								{
									...
									{
										if (vertPart != L & quot; &quot;)   // Check to see if there even is a tex coord
										{
									...
									vertPart = L & quot; &quot;;   // Get ready for next vertex part
						...
						if (checkChar == 'm')
						{
							...
							if (checkChar == 't')
							{
								...
								if (checkChar == 'l')
								{
									...
									if (checkChar == ' ')
									{
										meshMaterialsTemp = L & quot; &quot;;  // Make sure this is cleared
	...
	else    // If we could not open the file
	{
		...
		// create message
		std::wstring message = L & quot; Could not open: &quot;;
		message += Filename;
